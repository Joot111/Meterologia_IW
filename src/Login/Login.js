  ...
  const proceedLogin = async (e) => {
    ...
    if (validate()) {
      try {
        ...
        //const response = await fetch(`https://api.sheety.co/272ea837f9f1630e722be20ebe12f7b3/register/register?filter[email]=${email}&filter[password]=${password}`); -- original
        const response = await fetch(`https://api.sheety.co/143d3a99cbe53d62a0cc649674db7d79/register/register?filter[email]=${email}&filter[password]=${password}`); //-- substituto
        if (!response.ok) {
